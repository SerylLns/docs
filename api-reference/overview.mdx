---
title: API Overview
description: REST API to manage AI agents, WhatsApp messaging, and structured workflows.
---

<Info>
  **Base URL:** `https://whatsrb.com/api/v1` — All requests require `Authorization: Bearer YOUR_API_KEY`
</Info>

## Resources

<CardGroup cols={3}>
  <Card title="Agents" icon="brain">
    Create AI agents with system prompts, allowed actions, and structured fields. Run them on inbound messages or via API.
  </Card>
  <Card title="Agent Runs" icon="play">
    Trigger agent executions and inspect results — structured output, tokens, latency, and conversation context.
  </Card>
  <Card title="Business Accounts" icon="building-2">
    Connect WhatsApp Business accounts via Meta OAuth. Manage settings and human takeover duration.
  </Card>
  <Card title="Messages" icon="message-circle">
    Send text, images, documents, and template messages. Outbound messages auto-extend human takeover pauses.
  </Card>
  <Card title="Conversation Pauses" icon="hand">
    Pause AI agents per-conversation when a human takes over. Auto-expires, auto-extends on outbound.
  </Card>
  <Card title="Templates" icon="layout-template">
    Create, sync from Meta, and send pre-approved message templates with dynamic variables.
  </Card>
  <Card title="Webhooks" icon="webhook">
    Register endpoints for real-time events: agent runs, messages, statuses, pauses, and quota alerts.
  </Card>
  <Card title="Sessions" icon="smartphone">
    Manage WhatsApp Web sessions via Baileys bridge. For dev/testing — use Business Accounts in production.
  </Card>
  <Card title="Usage" icon="bar-chart-2">
    Monitor plan limits, message throughput, agent runs, and delivery metrics.
  </Card>
</CardGroup>

## Authentication

Every request must include your API key as a Bearer token:

```bash
Authorization: Bearer wrb_live_xxxxxxxxxxxxx
```

Generate your key from the [WhatsRB Dashboard](https://whatsrb.com/dashboard) under **Settings → API Keys**.

<Warning>
  Keep your API key secret. Never expose it in client-side code or public repositories.
</Warning>

## Response format

All responses return JSON. Successful responses wrap data in a `data` key:

```json
{
  "data": {
    "id": "agt_abc123",
    "name": "Support Agent",
    "model": "gpt-4o-mini",
    "active": true,
    "allowed_actions": ["support.refund.process", "support.ticket.create"]
  }
}
```

List endpoints include a `meta` key with pagination info:

```json
{
  "data": [...],
  "meta": {
    "total": 42,
    "plan_limit": 10
  }
}
```

## Error format

Errors return a JSON object with a human-readable `error` key:

```json
{
  "error": "Business account not found"
}
```

| Status | Meaning |
|--------|---------|
| `400` | Bad request — invalid parameters |
| `401` | Unauthorized — missing or invalid API key |
| `403` | Forbidden — plan limit reached |
| `404` | Not found |
| `422` | Validation error |
| `429` | Rate limited |
| `5xx` | Server error |
