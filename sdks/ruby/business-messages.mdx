...
```ruby
# With hash buttons (full control over IDs)
...
registry = WhatsrbCloud::EventRegistry.new

registry.on("message.received") do |data|
  if data["message_type"] == "interactive"
    meta = data["message_metadata"]
    case meta["id"]
    when "confirm" then process_confirmation(data)
    when "cancel"  then handle_cancellation(data)
    end
  end
end
```
...
| `message_type` | String | `text`, `template`, `image`, `video`, `audio`, `document`, `interactive` |
