...
agent.payload_schema              # => PayloadSchema
agent.payload_schema.fields       # => [Field, Field, ...]
agent.payload_schema.field_names  # => [:room_number, :description, :due_at, :priority]
agent.payload_schema.required     # => [:room_number, :description]
```
...
### Validating run payloads

Use `payload_schema.check(payload)` to validate action output against the schema:

```ruby
run = agent.run(input: { last_message: "Clean room 204 tomorrow" })

run.actions.each do |action|
  result = agent.payload_schema.check(action["payload"])

  result.valid?    # => false (description missing)
  result.missing   # => [:description]
  result.present   # => [:room_number, :due_at]
  result.to_h      # => { room_number: "204", description: nil, due_at: "2026-03-03T00:00:00", priority: nil }

  if result.valid?
    Task.create!(result.to_h)
  else
    notify_human("Missing fields: #{result.missing.join(', ')}")
  end
end
```
...
<Note>
  ...
  Always validate on your end with `check`.
</Note>
...
| `payload_schema` | PayloadSchema | Structured fields with `check(payload)`, `required`, `field_names` |
...
| `triggered_by` | String | Trigger source: `sdk`, `api`, `auto`, `auto_inbound` |
| `input` | Hash | Input data sent to the agent |
| `output` | Hash | Agent output (intent, confidence, actions) |
| `error` | String | Error message if failed |
| `model_used` | String | LLM model used for this run |
