...
| `confirmation_mode` | String | `auto` or `confirm` |
| `confirmation_timeout_minutes` | Integer | Timeout for unconfirmed actions (default 30) |
| `active?` | Boolean | Whether agent is active |
...
| `action_statuses` | Array | Per-action confirmation statuses (null in auto mode) |
| `actions_decided_at` | String | Timestamp when all actions were decided |
