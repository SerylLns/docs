...
- Once all actions are decided, a `agent_run.actioned` webhook is sent

No widget configuration is needed — the behavior is driven entirely by the agent's `confirmation_mode` setting. See [Action Confirmation](/guides/action-confirmation) for the full guide.
